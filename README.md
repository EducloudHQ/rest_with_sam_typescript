...
```bash
sam12$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam12$ sam local start-api
sam12$ curl http://localhost:3000/
```
...
```bash
sam12$ sam logs -n HelloWorldFunction --stack-name sam12 --tail
```
